...
class PageDecorator extends DataExtension
{
    ...
    {
        $dir = 'themes/' . Config::inst()->get("SSViewer", "theme") . '/scss/';
    ...
    {
        $dir = 'themes/' . Config::inst()->get("SSViewer", "theme") . '/js/';
    ...
    private function updateGruntfile($directory)
    {
        ...
        try {
            $contents = $this->__readFILE(ROOT . 'themes/' . Config::inst()->get("SSViewer", "theme") . '/.Gruntfile-bolierplate.js');

            $this->__writeFILE(ROOT . 'themes/' . Config::inst()->get("SSViewer", "theme") . '/Gruntfile.js', str_replace('modules: []', 'modules: ' . $files, $contents));
        } catch (Exception $e) {
            user_error($e, E_USER_WARNING);
        }
    }
